...
export default ({ app, pagehandler, sitemaphandler, searchhandler, packageJson, authorized, admin, cors, argv }) => {
  ...
  app.get('/system/version.json', admin, async (req, res) => {
    ...
    const securityVersions = await Promise.all(deps.filter(d => d.startsWith('wiki-security')).map(getVersion))
    const pluginVersions = await Promise.all(deps.filter(d => d.startsWith('wiki-plugin')).map(getVersion))
